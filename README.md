...
跟着以下 5 步, 你可以在本地完整复现 Lantern Agent 的运作流程。

### Step 1 · 克隆项目并安装依赖

先把代码拉到本地, 项目用 pnpm workspace 管理 monorepo, 首次安装会解析 `apps/web/` 的依赖树 (约 500MB)。
...
> 💡 需要 Node.js ≥ 20 · pnpm ≥ 10 · [onchainos CLI](https://github.com/okx/onchainos-skills/releases) 预先安装。
...
```bash
# X Layer 上的 Agentic Wallet (运行 `onchainos wallet login <email>` 登录)
OKX_WALLET_ADDRESS=0xb266dd8d...  # 登录后自动生成的 EVM 地址

# Polymarket 交易凭据 (如要真实下单 Polymarket)
POLY_PRIVATE_KEY=0x...             # 控制 funder 地址的私钥
POLY_SIGNER_ADDRESS=0xE14E6C...    # Signer 地址
POLY_FUNDER_ADDRESS=0xc78873...    # Funder 地址 (持有 USDC.e)
POLY_SIGNATURE_TYPE=2              # Gas-less 签名
POLY_CHAIN_ID=137                  # Polygon
POLY_HOST=https://clob.polymarket.com
```

> 🔒 `.env` 已被 gitignore, 不会提交到仓库。

### Step 3 · 跑一次完整 Agent 循环

让 Agent 扫描 245+ 个 Polymarket 价格预测市场 + 100 个 X Layer 代币, 计算 Edge 并产出决策:
...
PHASE 1: SCAN — Market Discovery
  → onchainos token hot-tokens --chain 196   ✓ 100 tokens
  → onchainos signal list --chain 196        ✓ 3 signals

PHASE 1.5: POLYMARKET — Price Markets Edge Scan
  Fetched 2560 unique candidate markets
  Found 25 markets with on-chain edge data

  Market                                 Poly%   Ours%    Edge
  Will Bitcoin hit $60k or $80k first?   35.5%   53.4%   +17.9% ★
  Will Bitcoin reach $85,000 in April?    5.9%    8.8%    +2.8%
  ...

PHASE 3: DECIDE — Bayesian Scoring
  Best recommendation: FDOG  Signal strength: 79.9%

PHASE 5: OUTPUT — Reasoning Trace
  Artifact saved: runtime-artifacts/demo/latest.json
  Cycle complete in 32.4s
```

所有数据 (25 个 edge 市场 + 19 次 onchainos API 调用 + 贝叶斯轨迹) 会写入 `runtime-artifacts/demo/latest.json`, 供 Showcase 展示页读取。

### Step 4 · 把决策上链 (零 Gas 费)

将这次循环的决策日志以 JSON 写入 X Layer, 永久公开可验证:
...
```
[1/3] Running agent scan cycle...

[2/3] On-chain record:
{
  "agent": "lantern",
  "chain": "xlayer-196",
  "recommendation": { "token": "FDOG", "probability": 0.799, "signals": 4 },
  "polymarkets": { "scanned": 249, "withEdge": 25 }
}

[3/3] Writing to X Layer (339 bytes)...
  ✅ Decision trace written to X Layer!
  TxHash: 0x6c7119352863367f912a003421449d38e90b2130d4d7806ea9734ac0d1d52ab6
  Explorer: https://www.okx.com/web3/explorer/xlayer/tx/0x6c71...
```

**零 Gas 费** · 每次 Agent 循环可免费归档, 这是 X Layer 原生优势。

### Step 5 · 实盘下单 Polymarket (可选)

如果 funder 地址有 USDC.e, 可以让 Agent 在 Polymarket 上真实下单:
...
Wallet: 0xE14E6C10e688Ab2C8aF3e60EdeB1Af71aD7ddFF1
Collateral balance: 4.958263 USDC.e ✓

Selected market: MicroStrategy sells any Bitcoin by December 31, 2026?
Placing order: BUY 8.33 YES @ $0.12

✓ Order matched
  Order ID: 0xade94b347dc99de5fe5038d72914f867d2eedacf1d6e2e62d466789677ce3127
  TxHash:   0x23872647d57ac1165a503fd1d954f14d618d895068e3aa339762c30615f3f490
  Filled:   $1.00 USDC → 8.33 YES shares
```

TxHash 可在 [Polygonscan](https://polygonscan.com/tx/0x23872647d57ac1165a503fd1d954f14d618d895068e3aa339762c30615f3f490) 验证。

### 本地预览 Showcase 网页

启动 Next.js 开发服务器, 查看 10 slides 的展示页:

```bash
pnpm dev
```

打开 [http://localhost:3000/showcase](http://localhost:3000/showcase), 用 `←` / `→` 箭头或底部圆点切换页面, 会读取 `runtime-artifacts/demo/latest.json` 中的真实数据。
...
**Q: onchainos CLI 未登录?**
先执行 `onchainos wallet login your@email.com` 完成邮箱 OTP 验证, 账户会自动创建。

**Q: X Layer 钱包余额 $0 但可以上链?**
是的, X Layer 零 Gas, `contract-call` 不需要余额就能成功 (已测试验证)。

**Q: Polymarket `pnpm poly:trade` 提示余额不足?**
`POLY_FUNDER_ADDRESS` 需要在 Polygon 上有 USDC.e (合约 `0x2791bca1f2de4661ed88a30c99a7a9449aa84174`)。最小 $1 可完成演示。

**Q: 想修改焦点市场?**
编辑 `apps/web/components/showcase/market-cards.tsx` 的 `FOCUS_MARKET` 常量, 从 `pnpm agent:demo` 产出的 25 个 edge 市场里挑一个替换。
