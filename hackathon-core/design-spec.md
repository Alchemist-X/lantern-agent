# Lantern Agent — 黑客松展示页面设计规范 v3

> 融合 Slay the Spire 视觉语言 + 灯笼隐喻，构建可交互的单页展示

---

## 项目一句话

**我们用 Onchainos 链上数据技能，让 AI Agent 在预测市场中自动发现定价偏差（edge），持续跟踪多个市场，7×24 小时自主交易。**

---

## 核心概念：Market Pulse

系统围绕 **Market Pulse** 这一核心组件设计：
...
```
              ...
              │  动态地从链上信息源收集证据                      │
              │  将其与市场隐含赔率对比                         │
              │  综合偏差值（edge）和回报周期（monthly return）    │
              │  给出交易指示                                  │
              │                                             │
              │    信息源          市场赔率                    │
              │      │               │                      │
...
**插画 A：复杂推理能力**（火柴人简笔画风格）

```
  左边：火柴人看着一堆数据（K线、新闻、链上数据），头上冒问号
       标注："人类：一次只能分析 1-2 个市场"

  右边：灯笼机器人同时看 10 个屏幕，头上是灯泡
       标注："Agent：同时监控数千个市场"
       
  底部：AI 在复杂任务上的推理能力已接近人类水平。
       人类的优势在于更好的信息源——但 Onchainos 弥合了这个差距。
```
...
```
  ...
  0s    新闻爆发 💥
  1s    Agent 已经读完链上数据，开始推理 [灯笼亮]
  3s    Agent 已完成概率计算，下单执行 [交易完成 ✓]
  ...
  180s  人类终于看到新闻，打开电脑 [火柴人刚坐下]
  240s  人类开始分析 [火柴人挠头]
  300s  人类还在犹豫要不要下单 [火柴人汗]
  
  底部：Agent 秒级响应 vs 人类 3-5 分钟。在预测市场中，这个时间差就是利润。
```
...
```
  ...
  人类：只有 8-10 小时在线（白天部分高亮，夜晚灰暗）
       标注："睡觉时错过了 3 个交易机会"
       
  Agent：24 小时全亮（灯笼在圆环中心持续燃烧）
       标注："每 60 秒一个循环，从不休息"
       
  底部：Agent 能 7×24 小时监控数千个市场，
       发现任何个人无法跟踪的定价失调。
```

### Onchainos 是什么

Onchainos 是 OKX 提供的链上数据操作系统，包含 14 个 Skill（技能模块），覆盖：
- **代币发现**（热门代币、趋势排名）
- **聪明钱追踪**（鲸鱼、KOL、Smart Money 实时交易流）
- **安全扫描**（蜜罐检测、合约风险评估）
- **价格数据**（实时价格、K 线、成交量）
- **交易执行**（500+ DEX 聚合交易）
- **钱包管理**（余额查询、签名、转账）

Lantern Agent 集成了其中 **7 个 Skill**，构建了从信号发现到交易执行的完整闭环。
...
Display:  64px / 1.0  Cinzel 700  letter-spacing 10px (首屏大标题)
H1:       40px / 1.1  Cinzel 700  (区块标题——大一号更有冲击力)
H2:       26px / 1.2  Cinzel 600  (子标题)
Body:     17px / 1.7  Inter 400   (正文——加大到 17px 提高可读性)
Data:     16px / 1.4  JetBrains Mono 700  (数据值)
Label:    13px / 1.3  Inter 500   uppercase  letter-spacing 2px  (标签)
Tiny:     12px / 1.3  Inter 400   (脚注)
```
...
在 Hero 和 Problem 之间插入一个简洁的项目说明区块，用大字 + 插画解释核心概念。
...
│  我们做了什么                                        │
│                                                     │
│  Lantern Agent 利用 Onchainos 链上数据技能            │
│  （聪明钱追踪、代币分析、安全扫描、价格数据），          │
│  让 AI 自主评估预测市场中事件发生的概率。               │
│                                                     │
│  当 Agent 计算出的概率与市场定价存在偏差时              │
│  ——这就是 Edge（套利空间）——Agent 自动下单。           │
│                                                     │
│  ┌──────────────────────────────────────────────┐   │
│  │          [插画 D: Edge 是什么]                 │   │
│  │                                              │   │
│  │  火柴人 A（市场）：                            │   │
│  │    "我觉得 BTC 涨到 $76K 的概率是 58%"         │   │
│  │                                              │   │
│  │  灯笼机器人 B（Agent）：                       │   │
│  │    "我看了链上数据——聪明钱在买、                 │   │
│  │     交易量放大、鲸鱼在囤——                      │   │
...
- 标题: Cinzel 40px, `#FFF6E2`
- 正文: Inter 17px, `#E0E0E0`, 行高 1.8（大字留白，好读）
- 插画: 内嵌的简笔画风格图，用 SVG 或纯 CSS 绘制
- 背景: `#0D1117` + 微弱灯笼光晕从顶部（延续 Hero 的光照）
...
- 标题: Cinzel 64px, weight 700, `#FFF6E2`, letter-spacing 10px
...
- 底部引言: Cinzel 24px, `#FFF6E2`, 居中, 下方 Inter 14px 来源

在对比列下方，加入**插画 A/B/C**（火柴人简笔画，上方"核心概念"章节中已定义），分别说明：
- 推理能力接近人类
- 秒级响应 vs 分钟级
- 24 小时覆盖 vs 8 小时
...
7 张 StS 风格卡牌，手牌式展开。每张卡牌 = 一个 Onchainos Skill = Agent 手中的一盏灯。

```
┌─────────────────────────────────────────────────────┐
│                                                     │
│  你的灯笼 — 7 盏灯，7 个维度                          │
│  7 个 Onchainos 技能 · 60 条命令 · 完整交易管线        │
│                                                     │
│       ┌────┐ ┌────┐ ┌────┐ ┌────┐                  │
│       │dex │ │dex │ │    │ │dex │                  │
...
从 `/api/demo-trace` 拉取真实数据，展示 Agent 最后一次循环的完整输出。这是页面的**高潮**——评委在这里看到 Agent 真的在思考。
...
1. **巡视灯笼** (S3): 灯笼图标沿 扫描→分析→决策→执行 路径移动, 4 秒一圈
2. **滚动提示** (S1): ↓ 字符上下浮动, 2 秒一圈, 用户滚动后消失
...
- **零外部依赖**：不用 framer-motion、chart.js、fullpage.js
- CSS @keyframes + Intersection Observer + requestAnimationFrame
- 字体：Cinzel + Inter + JetBrains Mono（Google Fonts）
...
5. 不用弹窗
6. 不做亮色/暗色切换——只有暗色
7. **灯笼 SVG 优先于 emoji**：用 SVG 灯笼替代 🏮 📈 📉。仅保留 🛡️ ❤️ ⚡ ★ 四种 emoji
8. 不用渐变文字
9. 不用 border-radius > 16px
10. 不放团队照片——这是 Agent 的展示，不是人的
